...
try {
  $statement = $conn->prepare(<<<query
      select
        ...
        is_featured,
        is_new,
        case when category_id is null then '' else category_id end as category_id,
        case when supplier_id is null then '' else supplier_id end as supplier_id
      from shop_products
      where id = ?
    query);
...
// Check supplier_id
  ...
  if ($_FILES['image']['error'] == UPLOAD_ERR_NO_FILE) {
    ...
    if (getimagesize($_FILES["image"]["tmp_name"])) {
      ...
      $isUploaded = move_uploaded_file($_FILES['image']["tmp_name"], __DIR__ . "/../../..$image.tmp");
      if (!$isUploaded) {
        $conn->close();
        die($twig->render($template, [
          'el'   => $el,
  ...
  if (
    ...
    try {
      ...
      $bindImage = $isUploaded ? $image : $oldImage;
      $statement->bind_param('ssssddiiiiiii',
        $_POST['product_name'],
        $bindImage,
      ...
      $conn->close();
      if ($isUploaded) {
        rename(__DIR__ . "/../../..$image.tmp", __DIR__ . "/../../..$image");
      }
      header('location: ./');
    } catch (mysqli_sql_exception $exception) {
      $conn->close();
      if ($isUploaded) {
        unlink(__DIR__ . "/../../..$image.tmp");
      }
      echo 'Xin lỗi, không thể truy vấn cơ sở dữ liệu.';
    }
  } else {
