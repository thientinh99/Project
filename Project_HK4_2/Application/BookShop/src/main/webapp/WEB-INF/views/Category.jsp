							...
							<table class="mb-0 table">
								...
								<tbody>
									<c:forEach var="list" items="${list}">
										<c:if test="${empty list.categoryId}">
											<h1 class="alert alert-warning" role="alert">Content not
												available</h1>
										</c:if>
										<c:if test="${not empty list.categoryId}">
											<tr>
												<td>${list.name}</td>
												<td>${list.status}</td>
												<td>${list.description}</td>
												<td><a href="${category}/${list.categoryId}"
													class="btn btn-primary">Update</a></td>
													<%--  <a
													href="${category}/delete/${list.categoryId}"
	...
	<script>
		$(document).ready(function() {
			$("body").on("click", ".page-item", function() {
				...
				$.ajax({
					url : "/shopEcommerce/api/LayDanhSachCategoryLimit",
					type : "GET",
					data : {
						start : start,
					},
