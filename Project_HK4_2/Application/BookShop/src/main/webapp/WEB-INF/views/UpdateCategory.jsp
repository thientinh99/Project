...
<link rel="stylesheet" type="text/css"
	href='<c:url  value="/resources/assets/main.css"/>' />
<c:url var="addNew" value="/dash	board/category/addNewCategory" />
						...
						<div class="card-body">
							...
							<div class="container">
								<form class="form-group" method="post" action="">
									<c:forEach var="listCategory" items="${list}">
										<div class="row ">
											<div class="col-sm-6">
												<input type="text" class="form-control" name="categoryId"
													placeholder="Name Category"
													value="${listAuthor.categoryId}" hidden> <input
													type="text" class="form-control" name="name" id="name"
													placeholder="Name Category" value="${listCategory.name}">
											</div>
											<div class="col-sm-6">
												<input type="text" class="form-control " name="description"
													placeholder="description" id="description"
													value="${listCategory.description}">
											</div>
										</div>
										<div class="col-sm-6">
											<select name="status" id="status">
												<c:if test="${listCategory.status ==true}">
													<option value="1">Enable</option>
													<option value="0">Disable</option>
												</c:if>
												<c:if test="${listCategory.status ==false}">
													<option value="0">Disable</option>
													<option value="1">Enable</option>
												</c:if>
											</select>
