...
<c:url var="addNew" value="/dashboard/publishingCompany/addNewPublishingCompany" />
							...
							<table class="mb-0 table">
								...
								<tbody>
									<c:forEach var="list" items="${list}">
										<c:if test="${empty list.publishingId}">
											<h1 class="alert alert-warning" role="alert">Content not
												available</h1>
										</c:if>
										<c:if test="${not empty list.publishingId}">
											<tr>
												<td>${list.publishingName}</td>
												<td>${list.address}</td>
												<td>${list.description}</td>
												<td><a href="${publishingCompany}/${list.publishingId}" class="btn btn-primary">Update</a>
													</td>					
													<%-- <a href="${publishingCompany}/delete/${list.publishingId}" class="btn btn-secondary">Delete</a> --%>
											</tr>
										</c:if>
									</c:forEach>
								</tbody>
	...
	<script>
		$(document).ready(function() {
			$("body").on("click", ".page-item", function() {
				...
				$.ajax({
					url : "/shopEcommerce/api/LayDanhSachPublishingCompanyLimit",
					type : "GET",
					data : {
						start : start,
					},
